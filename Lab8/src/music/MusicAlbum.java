...
public class MusicAlbum {
     ...
     * @param title string
     */
    public MusicAlbum(final String title) throws MusicException {
        if (title.length() > 1) {
            this.title = title;
            this.musicTracks = new MusicTrack[maxAmountOfRecords];
            this.counter = 0;
        } else {
            throw new MusicException("Invalid parameters for the music album.");
        }
    ...
    public MusicTrack get(final int position) {
        if (position < 0 || position > maxAmountOfRecords) {
            throw new MusicException("Position in the album out of range");
        }
    ...
    public boolean equals(final Object obj) {
        ...
        if (obj instanceof MusicAlbum) {
            MusicAlbum m = (MusicAlbum) obj;

            for (int i = 0; i < maxAmountOfRecords; i++) {
                if (!this.get(i).equals(m.get(i))) {
                    return false;
                }
            }
